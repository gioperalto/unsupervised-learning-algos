...
def red_wine_quality(method='km'):
    if method == 'km':
        ...
        # ICA
        c = 3
        x = FastICA(n_components=c, max_iter=1000).fit_transform(x)
        ...
        print('RWQ [ICA + k-means] time (ms):', run_time*1000)

        km_as = accuracy_score(kmeans.predict(x_test),y_test)
        print("k-means clustering accuracy score: ",km_as)

        # Plot
        plot_clusters(k, x_test, kmeans, 'dimen-reduction/ica/red-wine-k-means')
    elif method == 'em':
        # Read in Red Wine Quality
        ...
        run_time = time.process_time() - create_start - test_train_time
        print('RWQ [ICA + EM] time (ms):', run_time*1000)

        em_as = accuracy_score(em.predict(x_test),y_test)
        print("EM clustering accuracy score: ",em_as)

        # Plot
        plot_clusters(k, x_test, em, 'dimen-reduction/ica/red-wine-em')
    else:
...
def fish_market(method='km'):
    if method == 'km':
        ...
        # ICA
        c = 6
        x = FastICA(n_components=c).fit_transform(x)
        ...
        run_time = time.process_time() - create_start - test_train_time
        print('Fish market [ICA + k-means] time (ms):', run_time*1000)

        km_as = accuracy_score(kmeans.predict(x_test),y_test)
        print("k-means clustering accuracy score: ",km_as)

        # Plot
        plot_clusters(k, x_test, kmeans, 'dimen-reduction/ica/fish-market-k-means')
        ...
        run_time = time.process_time() - create_start - test_train_time
        print('Fish market [ICA + EM] time (ms):', run_time*1000)

        em_as = accuracy_score(em.predict(x_test),y_test)
        print("EM clustering accuracy score: ",em_as)

        # Plot
        plot_clusters(k, x_test, em, 'dimen-reduction/ica/fish-market-em')
    else:
        print('Invalid method: {}'.format(method))
...
if __name__ == "__main__":
    if len(sys.argv) > 3:
        ...
        print('Proper usage: ica.py <dataset> [red_wine_quality|fish_market] <method> [km|em]')
    elif len(sys.argv) < 3:
