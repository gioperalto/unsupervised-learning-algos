...
def red_wine_quality(method='km'):
    if method == 'km':
        ...
        print('RWQ [Isomap + k-means] time (ms):', run_time*1000)

        km_as = accuracy_score(kmeans.predict(x_test),y_test)
        print("k-means clustering accuracy score: ",km_as)

        # Plot
        plot_clusters(k, x_test, kmeans, 'dimen-reduction/isomap/red-wine-k-means')
    elif method == 'em':
        # Read in Red Wine Quality
        ...
        run_time = time.process_time() - create_start - test_train_time
        print('RWQ [Isomap + EM] time (ms):', run_time*1000)

        em_as = accuracy_score(em.predict(x_test),y_test)
        print("EM clustering accuracy score: ",em_as)

        # Plot
        plot_clusters(k, x_test, em, 'dimen-reduction/isomap/red-wine-em')
    else:
...
def fish_market(method='km'):
    if method == 'km':
        ...
        run_time = time.process_time() - create_start - test_train_time
        print('Fish market [Isomap + k-means] time (ms):', run_time*1000)

        km_as = accuracy_score(kmeans.predict(x_test),y_test)
        print("k-means clustering accuracy score: ",km_as)

        # Plot
        plot_clusters(k, x_test, kmeans, 'dimen-reduction/isomap/fish-market-k-means')
    elif method == 'em':
        # Read in Fish market
        fish_data = pd.read_csv('data/fish-market/full.csv')
        species = fish_data['Species'].value_counts().index.tolist()
        ...
        run_time = time.process_time() - create_start - test_train_time
        print('Fish market [Isomap + EM] time (ms):', run_time*1000)

        em_as = accuracy_score(em.predict(x_test),y_test)
        print("EM clustering accuracy score: ",em_as)

        # Plot
        plot_clusters(k, x_test, em, 'dimen-reduction/isomap/fish-market-em')
    else:
        print('Invalid method: {}'.format(method))
...
if __name__ == "__main__":
    if len(sys.argv) > 3:
        ...
        print('Proper usage: isomap.py <dataset> [red_wine_quality|fish_market] <method> [km|em]')
    elif len(sys.argv) < 3:
