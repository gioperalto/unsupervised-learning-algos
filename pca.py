...
def red_wine_quality(method='km'):
    if method == 'km':
        ...
        # Plot
        plot_clusters(k, x_test, kmeans, 'dimen-reduction/pca/red-wine-k-means')
    elif method == 'em':
        # Read in Red Wine Quality
...
def fish_market(method='km'):
    if method == 'km':
        ...
        # Plot
        plot_clusters(k, x_test, kmeans, 'dimen-reduction/pca/fish-market-k-means')
...
if __name__ == "__main__":
    if len(sys.argv) > 3:
        ...
        print('Proper usage: pca.py <dataset> [red_wine_quality|fish_market] <method> [km|em]')
    elif len(sys.argv) < 3:
